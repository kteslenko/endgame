...
static void render(t_scene *scene, t_renderer *renderer) {
    ...
    SDL_Rect text_size = text_rect(renderer, "YOU LOST", name_font);

    text_pos = (SDL_Point){center.x - text_size.w / 2, center.y - text_size.h / 2 - 100}; 
    render_text(renderer, "YOU LOST", name_font, text_pos, name_text_color);
    ...
    render_text(renderer, "RESTART", menu_scene->font, text_pos, start_text_color);
